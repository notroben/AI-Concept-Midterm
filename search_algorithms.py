    ...
    for i in range(len(path) - 1):
        ...
        for neighbor, attrs in graph.get(u, []):
            if neighbor == v:
                ...
                found_edge = True
                break
        if not found_edge: return float('inf') # Rute tidak valid
    return total_cost
...
def find_initial_solution(graph, start_node, end_node):
    ...
    return None # Tidak ditemukan rute
...
def hill_climbing_search(graph, start_node, end_node, cost_type, max_iterations=1000):
    ...
    nodes_explored = 1 # Hanya menghitung iterasi utama
    ...
    for i in range(max_iterations):
        ...
        if not possible_new_nodes:
            continue # Tidak ada tetangga baru untuk dicoba
